...
class LigTools {
 ...
 public:
  static vector<vector<TriggerInfo>> getLocationPool(
      ordered_map<int, triggerAtStation>& triggerPool);
  static void permuteVector(vector<vector<TriggerInfo>> triggers, vector<TriggerInfo>& current,
                            int index, vector<vector<TriggerInfo>>& CombinationPool,
                            unordered_map<int, unordered_map<int, double>>& siteTimeMap);
  static vector<vector<TriggerInfo>> getLocationPool_p(
      ordered_map<int, triggerAtStation>& triggerPool,
