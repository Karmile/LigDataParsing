...
struct Duration {
  ...
  long long second() const {
    return m_Year * 31536000 + m_Month * 2592000 + m_Day * 86400 + m_Hour * 3600 + m_Min * 60 +
           m_Sec;
  }
...
struct GPSTime {
  ...
  GPSTime(int input_Year, int input_Month, int input_Day, int input_Hour, int input_Min,
          int input_Sec, double input_ActPointSec = 0, bool isTimeVaild = true,
          bool isTimeConfirm = true) {
  ...
  friend bool operator==(const GPSTime& s1, const GPSTime& s2) {
    return (s1.m_ActPointSec == s2.m_ActPointSec) && (s1.m_Year == s2.m_Year) &&
           (s1.m_Month == s2.m_Month) && (s1.m_Day == s2.m_Day) && (s1.m_Hour == s2.m_Hour) &&
           (s1.m_Min == s2.m_Min) && (s1.m_Sec == s2.m_Sec);
  }
  friend bool operator<(const GPSTime& s1, const GPSTime& s2) {
    return ((s1.m_Year < s2.m_Year)) || ((s1.m_Year == s2.m_Year) && (s1.m_Month < s2.m_Month)) ||
           ((s1.m_Year == s2.m_Year) && (s1.m_Month == s2.m_Month) && (s1.m_Day < s2.m_Day)) ||
           ((s1.m_Year == s2.m_Year) && (s1.m_Month == s2.m_Month) && (s1.m_Day == s2.m_Day) &&
            (s1.m_Hour < s2.m_Hour)) ||
           ((s1.m_Year == s2.m_Year) && (s1.m_Month == s2.m_Month) && (s1.m_Day == s2.m_Day) &&
            (s1.m_Hour == s2.m_Hour) && (s1.m_Min < s2.m_Min)) ||
           ((s1.m_Year == s2.m_Year) && (s1.m_Month == s2.m_Month) && (s1.m_Day == s2.m_Day) &&
            (s1.m_Hour == s2.m_Hour) && (s1.m_Min == s2.m_Min) && (s1.m_Sec < s2.m_Sec)) ||
           ((s1.m_Year == s2.m_Year) && (s1.m_Month == s2.m_Month) && (s1.m_Day == s2.m_Day) &&
            (s1.m_Hour == s2.m_Hour) && (s1.m_Min == s2.m_Min) && (s1.m_Sec == s2.m_Sec) &&
            (s1.m_ActPointSec < s2.m_ActPointSec));
  }
  friend bool operator<=(const GPSTime& s1, const GPSTime& s2) {
    return (s1 == s2) || ((s1.m_Year < s2.m_Year)) ||
  ...
  friend double operator-(const GPSTime& s1, const GPSTime& s2) {
    if ((s1.m_Year == s2.m_Year) && (s1.m_Month == s2.m_Month) && (s1.m_Day == s2.m_Day) &&
        (s1.m_Hour == s2.m_Hour) && (s1.m_Min == s2.m_Min) && (s1.m_Sec == s2.m_Sec)) {
      return s1.m_ActPointSec - s2.m_ActPointSec;
    }
    return (s1.m_Year - s2.m_Year) * 1e10 + (s1.m_Month - s2.m_Month) * 1e8 +
           (s1.m_Day - s2.m_Day) * 1e6 + (s1.m_Hour - s2.m_Hour) * 1e4 +
           (s1.m_Min - s2.m_Min) * 1e2 + (s1.m_Sec - s2.m_Sec) + s1.m_ActPointSec -
           s2.m_ActPointSec;
  }
  ...
  GPSTime operator+=(const Duration& s2) {
    ...
    *this = GPSTime(tm_.tm_year - 100, tm_.tm_mon + 1, tm_.tm_mday, tm_.tm_hour, tm_.tm_min,
                    tm_.tm_sec, m_ActPointSec);
    return *this;
  }
  string str() const {
    char buffer[20];
    std::snprintf(buffer, sizeof(buffer), "%04d-%02d-%02d %02d:%02d:%02d", m_Year, m_Month, m_Day,
                  m_Hour, m_Min, m_Sec);
...
struct TriggerInfo {
  ...
  friend bool operator==(const TriggerInfo& s1, const TriggerInfo& s2) {
    return (s1.time == s2.time) && (s1.stationID == s2.stationID);
  }
