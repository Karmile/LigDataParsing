...
class CGPSTimeAlgorithm {
 public:
  ...
  static GPSTime CalculateActuallyTime(GPSTimeInformation OneGPSTime, unsigned long Count,
                                       unsigned long Place, unsigned long NumOfDateOfPerCache,
                                       double SampleRate) {
    GPSTime ActTime = OneGPSTime.m_currentTime;

    double Dvalue = ((double)Count - (double)OneGPSTime.m_currentSecondCacheCount) *
                        (double)NumOfDateOfPerCache +
                    ((double)Place - (double)OneGPSTime.m_currentSeconfCachePlace);
  ...
  static void GetTimeStr(GPSTime Time, char *str, int strLength) {
    sprintf(str, "%02d%02d%02d%02d%02d%02d.%08d", Time.m_Year, Time.m_Month, Time.m_Day,
            Time.m_Hour, Time.m_Min, Time.m_Sec, int(Time.m_ActPointSec * 100000000));
  }
  ...
  static GPSTime GPSTimeCarrtOneSecond(
      GPSTime OneTime)  // If the Point data of GPS >1, the time should be carried;
  {
    struct tm tm;
    memset(&tm, 0, sizeof(tm));
    tm.tm_year = OneTime.m_Year + 2000 - 1900;
  ...
  static double GetGPSTimeFromHour(GPSTime PointTime) {
    ...
    delete[] time;
    //_CrtDumpMemoryLeaks();
    return atof(s.c_str());  // ���Ժ�ת��Ϊdouble���͵�ʱ��
  }
